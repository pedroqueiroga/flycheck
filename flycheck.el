...
(defun flycheck-eslint-config-exists-p ()
  ...
  (eq 0 (flycheck-call-checker-process
...
CHECKER's executable is assumed to be a Python REPL."
  (-when-let* ((script (concat "import sys; sys.path.pop(0);"
                               (format "import %s; print(%s.__file__)"
                                       module module))))
    (with-temp-buffer
      (and (eq 0 (flycheck-call-checker-process checker nil t nil "-c" script))
...
(defun flycheck-racket-has-expand-p (checker)
  ...
  (eq 0 (flycheck-call-checker-process checker nil t nil "expand")))
